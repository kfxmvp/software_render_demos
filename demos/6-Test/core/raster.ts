...
import { FrameBuffer, StencilType } from "./buffer";
...
export class Raster {
    ...
    private _useZTest: boolean = true;
    public set useZTest(use: boolean) {
        this._useZTest = use;
    }
    public get useZTest(): boolean {
        return this._useZTest;
    }

    private _useStencil: boolean = false;
    public set useStencil(use: boolean) {
        this._useStencil = use;
    }
    public get useStencil(): boolean {
        return this._useStencil;
    }

    private _stencilType: StencilType = StencilType.Always;
    public set stencilType(type: StencilType) {
        this._stencilType = type;
    }
    public get stencilType(): StencilType {
    ...
    private _raster(vf1: Vert2Frag, vf2: Vert2Frag, vf3: Vert2Frag, shader: Shader) {
        ...
        for (let x = xMin; x < xMax; ++x) {
            for (let y = yMin; y < yMax; ++y) {
                ...
                //模板测试
                if (this._useStencil && !this._buffer.stencilTest(this._stencilType, x, y)) continue;

                //深度测试
                if (this._useZTest && !this._buffer.zBufferTest(x, y, v2f.windowPosition.z)) continue;

                const renderColor = shader.fragmentShader(v2f);
                if (!renderColor) continue;
