...
export class Light {
    ...
    public calc(viewDir: Vec4, lightDir: Vec4, normal: Vec4): Color {
        // 反射方向
        const reflectDir = CalcUtil.reflect(lightDir.clone().mul3(-1), normal);

        // 环境光
        const ambient = this._useAmbient
            ? this._ambientColor.clone().mul3(this._ambientIntensity)
            : Color.WHITE.clone();
    ...
    constructor() {
        ...
        // 高光设置
        this._specularColor = Color.WHITE.clone();
